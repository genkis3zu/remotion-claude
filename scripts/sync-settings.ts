...
const OUTPUT_PATH = path.join(ROOT_DIR, "src", "voicevox-template", "settings.generated.ts");
  ...
  for (const character of characters) {
    ...
    const scanDir = (dir: string, baseRelative = ""): string[] => {
      ...
      for (const entry of entries) {
        ...
        const relPath = baseRelative ? path.join(baseRelative, entry.name) : entry.name;

        if (entry.isDirectory()) {
          results = results.concat(scanDir(fullPath, relPath));
        } else if (entry.isFile() && entry.name.endsWith(".png")) {
          // Normalize path separators to forward slashes for usage in code
    ...
    availableImages[character] = scanDir(charDir);
  }
...
`;
  ...
  console.log("✅ src/voicevox-template/settings.generated.ts を生成しました");
}
